...
状态：P2 必要流程已通过验收。双版本完整入口 `artifacts/p2-6716e7c27dd14c0599dc4c568de4cbcd/verification.json` 通过 7684 项；当前构建另通过隔离安装 46 项及协议 27 项，含明确指定端口冲突，证据为 `artifacts/installation-2349d95da47b4d95823bc98c70e0c3b2/verification.json`。测试源码、构建哈希及分项证据见 [覆盖清单](COVERAGE.md#当前验证证据)。P2 不承诺穷举资源不足、完整 GPIF 模型语义、恢复期间外部状态改变及所有模态组合；这些继续纳入 P7 可靠性验证，无可信终态时保留写入阻塞。
...
3. 继续 P3 具名曲谱/音轨操作。P7 扩展资源不足、模板初始化异常、恢复期间外部改动、复杂曲谱和更广泛模态组合，不把这些组合的未穷举结果表述为已验证。
...
当前显式标签恢复核心在 Windows PowerShell 5.1 与 PowerShell 7 下分别完整通过十六组、2574 项回归，其中包含 281 项标签与原生菜单检查；两个版本另各通过显式标签恢复 330 项、部分文档关闭后核验 327 项，并在 Windows PowerShell 5.1 通过原生保存故障恢复 265 项。标签回滚、保存异常恢复、窗口恢复、另存修复、标签重排及后台 DDE/连接 61 项、可见 DDE/连接 60 项仍作为历史基线保留。路径、二进制哈希和启动等待条件见下方记录及 [覆盖清单](COVERAGE.md#当前验证证据)。

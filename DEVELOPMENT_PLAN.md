...
| P2.7 关闭与共同编辑 | 明确保存/丢弃/取消，采用保守默认；验证手工编辑、标签重排、多份未保存文档、过期 ID、模态对话框及关闭最后文档 | 基础关闭、插件重排、另存标签更新、反复恢复窗口后的原生菜单和关闭确认取消已有专项验证；用户确认原生拖动不改变顺序，其他共同编辑及模态上下文待完成 |
...
2. 继续 P2 文档标签与共同编辑：插件 `gp_move_document` 的重排、撤销、保存重开、关闭、反复恢复窗口后的原生菜单及另存后的路径/标签更新已有专项验证；补齐同步重排校验失败后的恢复和其他手工/模态上下文。用户确认直接拖动不改变顺序，原生拖动保持独立的宿主限制项，不能用插件重排通过替代其验收。
...
当前保存异常恢复核心在 Windows PowerShell 5.1 与 PowerShell 7 下分别完整通过十六组、2574 项回归，其中包含 281 项标签与原生菜单检查，另分别通过原生保存故障恢复 264 项。窗口恢复、另存修复、标签重排及后台 DDE/连接 61 项、可见 DDE/连接 60 项仍作为历史基线保留。路径、二进制哈希和启动等待条件见下方记录及 [覆盖清单](COVERAGE.md#当前验证证据)。

...
3. P3、P4 已按必要范围完成，下一阶段为 P5 播放与音频。P4 的系统剪贴板及其他保留边界见上文。P7 扩展资源不足、模板初始化异常、恢复期间外部改动、复杂曲谱和更广泛模态组合，不把未穷举结果表述为已验证。
...
6. 继续按顺序完成 P5、P6，每个阶段验收后提交，再在最终发布包上执行 P7。按实际受影响阶段报告阻塞，安装只是完整项目的一部分。

尚未验证的私有接口不足以支撑固定日历工期。按具名操作验收和阶段门槛衡量进展，依据用户确认的剩余操作清单重新估算工作量。

## 当前证据与历史基线

当前 P3 验收构建在 PowerShell 7.6.5 通过 19 组、3732 项完整回归并正常退出；准确源码、测试脚本和二进制哈希见 [覆盖清单](COVERAGE.md#p3-验收)。P2 双版本验收及下方各检查点保留各自的构建和验证范围，不将历史专项结果转移到当前构建。

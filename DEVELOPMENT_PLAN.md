...
3. P3、P4、P5、P6 已按必要范围完成，下一阶段为 P7 发布验收。系统剪贴板、音频及文件交换扩展边界见上文。P7 扩展资源不足、模板初始化异常、恢复期间外部改动、复杂曲谱和更广泛模态组合，不把未穷举结果表述为已验证。
4. 保留已验证的原生 DDE 流程，确认独立 GUI 进程的受支持边界；继续保留端口占用、显式端口、双客户端、过期身份、重连重启和清理检查。第二进程正常退出不能证明所请求文档已打开。
5. P1 真实更新、启动入口和安装生命周期已完成。P7 在最终发布包上重新验证，并处理快速退出挂起及更广泛的升级、恢复场景。
6. P6 按用户要求跳过受权限限制的完整回归并提交阶段记录；后续在最终发布包上执行 P7，不将本次跳过视为回归通过。

...
更新日期：2026-09-08。本计划记录可安装、随 Guitar Pro 启动自动加载的原生 MCP 插件从 P0 到 P7 的目标、验收标准和历史检查点。当前阶段已按 [覆盖清单](COVERAGE.md) 中声明的范围完成；未列入范围的扩展、实验功能和宿主限制仍在对应章节保留。实现方式沿用 `ponytail ultra`，优先复用现有 C++/Qt 运行环境和宿主原生接口。

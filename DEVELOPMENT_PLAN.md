...
| P2.7 关闭与共同编辑 | 明确保存/丢弃/取消，采用保守默认；验证手工编辑、标签重排、多份未保存文档、过期 ID、模态对话框及关闭最后文档 | 基础关闭、插件重排及新宿主中的原生菜单已有专项验证；用户确认原生拖动不改变顺序，反复隐藏后的菜单上下文、标签显示及全部模态上下文待完成 |
...
2. 继续 P2 文档标签与共同编辑：插件 `gp_move_document` 的重排、撤销、保存重开、关闭及新宿主原生菜单已有专项验证；调查反复隐藏/恢复后菜单动作可能保持禁用的上下文，并修复未命名文档另存后标签名称/提示可能仍为空的问题。用户确认直接拖动不改变顺序，原生拖动保持独立的宿主限制项，不能用插件重排通过替代其验收。
...
当前标签重排核心在 Windows PowerShell 5.1 与 PowerShell 7 下分别完整通过十六组、2469 项回归，另通过含原生菜单的标签专项 221 项和原生保存故障恢复 167 项。`09185ab` 的十五组、2276 项及后台 DDE/连接 61 项、可见 DDE/连接 60 项仍作为历史基线保留。路径、二进制哈希和启动等待条件见下方记录及 [覆盖清单](COVERAGE.md#当前验证证据)。

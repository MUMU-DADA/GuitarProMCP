...
| P2.6 保存与恢复 | 当前路径保存、保存副本、另存并采用新路径、明确覆盖；失败后恢复文件/路径/未保存状态；新文档必须指定目的地 | 常规保存、两种路径及界面名称同步、原生写入和保存/路径通知异常恢复已有验证；恢复再次异常时保留备份及写入阻塞；后续状态协调和完整原生保存进度取消待完成 |
| P2.7 关闭与共同编辑 | 明确保存/丢弃/取消，采用保守默认；验证手工编辑、标签重排、多份未保存文档、过期 ID、模态对话框及关闭最后文档 | 基础关闭、插件重排及故障回滚、显式恢复重试、部分文档已关闭后的状态核验、另存标签更新、反复恢复窗口后的原生菜单和关闭确认取消已有专项验证；用户确认原生拖动不改变顺序，复杂文档集变化、其他共同编辑及模态上下文待完成 |
...
原生部分写入故障、保存后校验恢复、保存错误对话框、保存/路径通知异常恢复及恢复再次异常时备份保留已有验证。独立 GUI 多进程、完整保存进度取消、手工标签重排、真实客户端配置重载和其他未知原生结果恢复仍未完成，P2 尚未验收。
...
2. 继续 P2 文档标签与共同编辑：插件 `gp_move_document` 的重排、撤销、保存重开、关闭、原生菜单、另存后的路径/标签更新，以及同步重排校验/通知失败回滚已有专项验证。`gp_recover` 已提供显式标签恢复及部分文档关闭后的核验；继续处理复杂文档集变化和其他手工/模态上下文。用户确认直接拖动不改变顺序，原生拖动保持独立的宿主限制项，不能用插件重排通过替代其验收。

...
| P9 编辑面板剩余能力 | 节拍文本、`PPP`–`FFF` 力度标记、`Upward`/`Downward`/`auto` 符干方向、`G2/F4/C3` 谱号写入和能力矩阵已按最小可核验范围完成；和弦/歌词/页面元数据及已有音色效果、速度自动化沿用 P5/P8；力度清除、力度/表情自动化、细粒度排版、完整歌词/刻谱排版、任意乐器/指法、自定义音色/效果、完整自动化、系统剪贴板、任意音轨映射、剩余特别粘贴过滤项及全部技法组合继续归入 P9，分别标为宿主受限、实验性或未实现，详见 [P9 剩余要求](DEVELOPMENT_PLAN.md#p9剩余要求归档) |
...
| 设置与工作区 | P6 页面、谱表及明确作用域偏好按最小范围完成；P6 阶段的完整回归曾因权限限制跳过，P7 已复验发布包中的适用路径。P6 页面尺寸和显示设置不进入宿主撤销栈，P8 页面元数据可整组撤销；视图缩放不承诺跨重开保持，任意样式、插件管理和更多偏好未扩展 |
| 完整状态 | 按真实用户操作建立清单，为每项状态/修改建立读回和持久化验证 |
| 兼容性与可靠性 | 多文档、并发、长时间运行、取消、异常路径、宿主升级，以及安装和卸载体验 |

## 当前验证证据

P9 是当前源码构建的最新功能验收，P8 与 P7 分别保留此前源码和最近安装包的结论。下面的历史阶段段落保留当时的证据和二进制哈希，不替代最新 DLL 的验证，也不重新定义当前范围。

### P9 验收

状态：完成可核验范围并新增实验性音轨自动化保留路径（2026-09-09）。使用 Windows x64 Guitar Pro 8.1.1.17 隔离开发宿主，实际加载当前构建的 `guitarpro_mcp.dll`，SHA-256 见专项证据。

| 检查 | 结果与具名覆盖 | 证据 |
| --- | --- | --- |
| P9 专项 | 65 项通过，`complete=true`；能力矩阵、`gp_capabilities` 公共矩阵、整轨自动化读取、保留 `Sound` 点的实验性 DSP 参数写入/不变写入/撤销重做/删除、节拍文本光标/选区写入、Unicode/空白、清除、`PPP`–`FFF` 力度标记光标/选区写入、`Upward`/`Downward`/`auto` 符干方向光标/选区写入、`G2/F4/C3` 谱号写入、dirty、撤销/重做、保存重开及关闭清理 | `artifacts/native-p9-1666ae49cb204e44a9c18183b6440c9c/verification.json` |
| P9 后完整原生回归 | 24 组、4406 项通过，`complete=true`，退出码 0，连接描述已清理；包含 P9 专项及 P0–P8 既有回归 | `artifacts/regression-6b51102da11049a0b4fcfb39902da538/regression.json` |

本次 `0.4.0` 构建核心 DLL SHA-256 为 `EE23C06BC74896C246EA8600ECEF38EADEC051A1B97A2E58A70CE88AF50F502D`，自动加载器为 `1B436DFCE03136E3C40F821689AB1617340F2E720599BAA17B451B3011695E73`，宿主为 `B233B0F1C87DEB3AECE693D51E8D3C3A841C88FEE78828607B20034737C4C6DF`。完成范围是原生节拍文本、节拍力度标记、符干方向、谱号写入路径、整轨自动化读取和保留既有点/旁路的实验性 DSP 参数路径及能力矩阵。`gp_p9_status` 的状态值为“已实现、已验证、实验性、未实现、宿主受限”，并把沿用的 P5/P8 能力和剩余边界逐项列出。P9 后完整 `test-all.ps1` 已通过 24 组、4406 项，PowerShell 语法检查和 `git diff --check` 已通过。

### P8 验收

状态：已完成（2026-09-08），按 [AGENTS.md](../AGENTS.md) 的最小必要范围验收。使用隔离的 Windows x64 Guitar Pro 8.1.1.17 开发宿主，实际加载 `.tools/native/plugins/generic/guitarpro_mcp.dll`，SHA-256 为 `52541545944091638DBFEA5BCEF7FA8C859D890DCD2976440F99681CCBA49DD4`。下列专项和完整回归使用同一 DLL，报告记录的源码哈希均与验收工作树一致。

| 检查 | 结果与具名覆盖 | 证据 |
| --- | --- | --- |
| P8 独立专项及 PDF 复核 | 207 项通过，`complete=true`、`faults=true`、`render_pdf=true`；保存重开、两页 PDF 文本提取及页面渲染通过 | `artifacts/native-p8-facf5a2c6a464a32ab2e726565d35b6a/verification.json`；同目录 `input.json`、`export.json`、`score.gp`、`score.txt`、`pages.pdf`、`pages.txt`、`render.png` |
| 最终构建完整原生回归 | 23 组、4375 项通过；包含 P8 203 项（未启用独立 PDF 文本分支，额外检查既有多文档隔离）、音频 PCM 100 项、P6 116 项及全部既有回归组；退出码 0，连接描述已清理 | `artifacts/regression-501eea73a4f44b96a2c8086dbf191a7e/regression.json`；P8 子组为 `artifacts/native-p8-920ad9d1590141cea088bc05f3d7ee92/verification.json` |
| 音频及文件交换回归 | 当前构建 PCM、速度/反复/播放定位、文件交换及 PDF/PNG/WAV 输出通过 | `artifacts/native-audio-dd85922702514449a847390a395c2cdf/verification.json`、`artifacts/native-p6-37699940840f44079544d7c97f1eaabb/verification.json` |

专项具名覆盖：

- 批量创建、实际 JSON 导入及无变化往返，三种 riff 模式及 spec 的追加/插入；混合吉他/钢琴/鼓组、钢琴双谱表、多声部和空声部、Drop D、拍号/调号与反复时间线。
- 跨小节九音延音链、连奏、连音、弯音/滑音/泛音及无序钢琴和弦的技法定位。输入按弦或 MIDI 身份核对；钢琴/鼓组禁止物理弦品和调弦字段。
- 和弦名称、根音/低音、m7 类型与和弦图、移除、首品、指法和横按保存；实际 notes 独立，钢琴谱表隔离。五行中文歌词保留空格/连字符，支持清除并在保存重开和 PDF 中读回。
- 段落设置/移除、起止和结构摘要；页面整组设置、一次撤销/重做、保存重开。PDF 确认标题、作者、作曲者、版权、页眉页脚及页码，PNG 复核实际页面和和弦图。
- 每类编辑的 dirty、一次 Undo/Redo、非法输入后模型不变、错误目标拒绝、双客户端顺序、等待取消；提交前/后注入异常时保留阻塞，`gp_recover` 只观察实际模型并确认 `not_applied/applied`，不重放编辑。
- 最终 GPIF、文件字节、保存重开及其他文档身份/内容/dirty 保持。两份报告均记录具体操作请求和真实终态。

实现复用原生 `Score::copyFrom` 副本及 `Score::replaceScore`，没有新增曲谱文件生成器、外部服务或运行时依赖。开发 PDF 文本复核使用 Python+pypdf 和 Poppler，均不进入产品运行链路。歌词避免原生高层 setter 的跨拍分词，最后在独立 Beat 上设置字面片段；横按采用可随 GP 文件保存的原生 Fingering 两端指法，未使用仅在内存中有效的图形横按。

保留边界：JSON v1 不是完整 GP 文件备份，未覆盖任意音色/效果定义、所有自动化及歌词/刻谱样式；ASCII 不可逆；全部自定义音程、技法和跳转的组合未穷举。原生读回未采纳输入时拒绝提交，Undo 后 dirty 是否清零遵循宿主行为。和弦类型仅枚举已核验的 0..28；早期探针中的越界枚举崩溃、歌词分词及图形横按不持久化均已修正，失败运行不计为通过。

本次没有更新正式安装目录、重跑 P7 一小时长测或把新 DLL 作为安装包重新发布；P7 的包哈希和安装生命周期证据仍只适用于该历史发布包。接口、边界和复现命令见 [P8 原生编曲与语义 JSON](../native/P8.md)。

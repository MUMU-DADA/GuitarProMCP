...
更新日期：2026-09-08。P0–P8 已按已声明范围完成，P9 扩展已规划但尚未实施或验收；未实现、实验性和宿主限制仍在各节明确列出。P8 当前源码构建与 P7 历史发布包分别记录证据，以下不以 DLL 加载成功、菜单可枚举或导出符号存在代替功能完成。
...
| P9 编辑面板剩余能力 | P8 已提供和弦/歌词基础；剩余文本、力度与谱号、谱面排版、任意乐器与指法、任意音色/效果、完整自动化、系统剪贴板和剩余组合按 [P9 计划](DEVELOPMENT_PLAN.md#p9编辑面板剩余能力) 管理，扩展尚未实施或验收 |
...
P8 是当前源码构建的功能验收，P7 保留最近安装包的发布结论。下面的历史阶段段落保留当时的证据和二进制哈希，不替代 P8 新 DLL 的验证，也不重新定义当前范围。

...
更新日期：2026-09-08。本文件记录 P0-P7 阶段计划、验收标准和执行历史，并集中记录当前 P8/P9 的实施计划。当前协作规范、开发目标、产品要求、范围决策和质量门槛统一见 [AGENTS.md](../AGENTS.md)；P8/P9 的实施细节和验收拆分维护在本文件。

> 维护规则：P8/P9 实施细节、子阶段和计划验收更新本文件；当前能力状态和最新实证更新 `AGENTS.md`、`COVERAGE.md` 或 `native/README.md`。未实现的计划内容不得写入已验证能力。
...
| P8 高层编曲与曲谱语义 | 已规划，尚未实现或验收 | 批量建谱、JSON 往返、文本六线谱、歌曲结构、和弦、歌词、段落和页面元数据；详细计划见下文 |
| P9 编辑面板剩余能力 | 已规划，尚未实现或验收 | 补齐当前编辑面板中未覆盖的歌词/和弦图/文本、力度与谱号、排版、任意乐器与指法、任意音色/效果、完整自动化、系统剪贴板及剩余组合；详细计划见下文 |

P0、P1、P2 为 P3–P6 提供基础，随后由 P7 对完整安装包进行发布验收。P1 已完成真实安装集成验收；可靠性检查继续贯穿后续阶段。

## P8：高层编曲与曲谱语义接口

P8 参考 `wegitor/guitar-pro-mcp` 的文件交换能力和 `quinnjr/guitar-pro-mcp` 的批量 GP6 生成接口，目标是降低 AI 编曲时的调用次数，并补齐当前原生工具缺少的曲谱语义对象。P8 仍以 Guitar Pro 原生 GPCore 为唯一业务模型，不复制 Python/PyGuitarPro 或 TypeScript GP6 writer 的第二套模型。

### P8.1 批量建谱

新增 `gp_create_from_spec`、`gp_apply_spec` 和 `gp_insert_tab` 规划接口。嵌套规格需要覆盖元数据、多个音轨、谱表、声部、小节、拍号、调号、节拍、音符、调弦、技法和速度；简单 riff 接口接受类似 `0-2-2` 的文本表示。写入模式为 `replace`、`append`、`insert`，由工具在进入宿主前完成结构校验和上限校验。

实现必须复用现有文档请求、原生撤销和恢复机制，不能直接拼接 GP 文件字节。批量失败时不得留下部分未报告的写入；需要返回可观察的请求终态，保存后核对 GPIF，重新打开后核对模型。

### P8.2 JSON、六线谱和结构摘要

新增 `gp_export_json`、`gp_import_json`、`gp_export_tab` 和 `gp_structure` 规划接口。JSON schema 需要稳定表达音轨、谱表、声部、节拍、音符、发声音高、弦、品位、时值、休止、技法、连音、延音线、反复、跳转和速度点，并支持导出后再导入的可验证往返。

`gp_export_tab` 输出结构化的 ASCII 六线谱，必须明确调弦、音轨、小节边界、声部和不可表示的技法，不得把格式化文本当作可逆来源。`gp_structure` 输出统一的歌曲结构摘要，供 AI 在修改前定位小节、段落和标记。

### P8.3 和弦与歌词

新增 `gp_read_chords`、`gp_edit_chord`、`gp_read_lyrics` 和 `gp_edit_lyrics` 规划接口。和弦对象至少覆盖名称、根音、类型、扩展音、转位/低音、首品、弦位、横按和指法；歌词对象至少覆盖行、音节或文本片段与小节/拍的关联。

这些接口必须区分和弦符号与实际音符和弦，支持多音轨、多声部和钢琴谱表隔离。自动指法搜索、自动重排和声学效果不作为 P8 默认完成条件。

### P8.4 段落和页面元数据

新增 `gp_read_sections`、`gp_edit_section` 规划接口，并扩展现有 `gp_presentation`。段落对象覆盖 Intro、Verse、Chorus、Bridge、Solo、Outro、Coda 等名称、起止小节、文本和可用标记；页面元数据覆盖页眉页脚、页面标题、作者、作曲者、版权和页码。

页面和段落写入必须证明实际进入宿主业务模型并在保存重开后读回；单纯修改 Qt 控件属性、生成临时显示文本或枚举菜单不计为完成。任意排版样式、符杠/括号布局和完整打印系统不属于 P8 默认范围。

### P8.5 验收顺序与质量门槛

1. 先完成 MCP schema、输入校验和只读导出，再接入批量写入。
2. 为批量写入建立单文档、多文档、空声部、钢琴双谱表、打击乐、跨小节和跨音轨夹具。
3. 每个写入工具核对目标文档隔离、未保存状态、撤销/重做、保存后 GPIF、重新打开和失败恢复。
4. 对 JSON 往返、ASCII 六线谱、结构摘要、和弦、歌词和页面元数据分别记录可读回字段与不可表示边界。
5. 在双客户端和未知异步结果场景下验证请求顺序、阻塞策略和错误目标拒绝；未通过的能力保持“未实现”或“实验性”状态。

P8 不改变当前支持的 Windows x64 Guitar Pro 8.1.1.17、原生 HTTP MCP、Qt 主线程和私有 ABI 约束。离线独立生成器、外部 Python/Node 常驻服务、任意乐器定义、自动指法搜索和完整排版系统不属于本阶段默认交付。

...
更新日期：2026-09-10。本文件记录 P0-P11 阶段计划、当前验收标准、P9 实施边界、P10 实施范围和 P11 实施范围及实验性边界。当前协作规范、开发目标、产品要求、范围决策和质量门槛统一见 [AGENTS.md](../AGENTS.md)；历史执行记录见文末归档。
...
- 活动模态对话框必须优先捕获实际阻塞用户操作的对话框，并与 `gp_dialogs` 的消息、按钮和模态状态交叉核对；对话框不存在时才捕获主窗口。
...
- 已交付 `gp_screenshot` 工具注册、MCP image 响应、API/README 说明和 PowerShell 专项脚本；当前专项证据覆盖正常、后台、最小化和焦点保持，真实宿主活动模态、遮挡、多文档及无文档矩阵仍需补验，因此能力标记为“实验性”，失败状态使用“宿主受限”。

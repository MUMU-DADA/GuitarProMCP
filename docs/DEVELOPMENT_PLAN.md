# GuitarProMCP 阶段计划与验收历史

更新日期：2026-09-08。本文件记录 P0-P8 阶段计划、验收标准和执行历史，并集中记录当前 P9 的实施计划。当前协作规范、开发目标、产品要求、范围决策和质量门槛统一见 [AGENTS.md](../AGENTS.md)；P8 验收拆分和 P9 实施计划维护在本文件。

> 文档分工遵循 [AGENTS.md](../AGENTS.md#文档分工)：阶段记录见本文件，当前能力及具名证据见 [COVERAGE.md](COVERAGE.md)，接口细节见 [native/README.md](../native/README.md) 和 [native/P8.md](../native/P8.md)。
...
| P9 编辑面板剩余能力 | 已规划，扩展尚未实施或验收 | 沿用 P8 的和弦/歌词基础，补齐剩余文本、力度与谱号、排版、任意乐器与指法、任意音色/效果、完整自动化、系统剪贴板及剩余组合；详细计划见下文 |

P0、P1、P2 为 P3–P6 提供基础，随后由 P7 对完整安装包进行发布验收。P1 已完成真实安装集成验收；可靠性检查继续贯穿后续阶段。

## P8：高层编曲与曲谱语义接口

状态：已完成（2026-09-08）。P8 参考 `wegitor/guitar-pro-mcp` 的文件交换能力和 `quinnjr/guitar-pro-mcp` 的批量 GP6 生成接口，降低编曲调用次数并补齐曲谱语义对象。实现以 Guitar Pro 原生 GPCore 为唯一业务模型，复用既有命令、文档请求和撤销机制。接口和 JSON 字段见 [P8 原生编曲与语义 JSON](../native/P8.md)，当前范围以 [AGENTS.md](../AGENTS.md) 为准。

### P8.1 批量建谱

已实现 `gp_create_from_spec`、`gp_apply_spec` 和 `gp_insert_tab`。嵌套规格覆盖元数据、多音轨、谱表、声部、小节、拍号、调号、节拍、音符、调弦、既有技法和速度；riff 接受 `0-2-2-r|3-5`。`replace/append/insert` 在原生副本中构建，结构和上限预检后逐字段核对读回，通过 `Score::replaceScore` 一次提交。未给出的元数据和主小节属性沿用原模型，音轨配置复用模板或原轨；插入模式拒绝更改已有音轨配置。

已验证混合吉他/钢琴/鼓组、多声部和空声部、钢琴双谱表、Drop D、跨小节连接链、批量一次撤销/重做、未保存状态、GPIF 和保存重开。新建在打开新标签前完成原生规格预检；非法规格不进入实时文档。异步请求沿用 `gp_operation/gp_cancel/gp_recover`，未知提交保留阻塞，恢复只核对提交前及预期状态。

### P8.2 JSON、六线谱和结构摘要

已实现 `gp_export_json`、`gp_import_json`、`gp_export_tab` 和 `gp_structure`。交换标识为 `guitarpromcp.p8` v1，表达音轨、谱表、声部、节拍、音符、发声音高、弦、品位、时值、休止、既有技法、连音、连接链、反复、跳转和速度点。已验证真实导入、导出后无变化往返、无序钢琴和弦的技法定位及保存重开；键盘/鼓组不输出或接受物理弦品字段。

ASCII 输出标明调弦、变调夹、声部、小节、时值和不可表示清单，已核对读操作不移动光标；文本不是可逆来源。结构摘要统一返回主小节、段落、标记、音轨和速度。JSON v1 限 32 轨、256 小节、20000 拍，HTTP 保留 1 MiB 上限；它不备份完整 GP 音色、自动化和排版状态。

### P8.3 和弦与歌词

已实现 `gp_read_chords`、`gp_edit_chord`、`gp_read_lyrics` 和 `gp_edit_lyrics`。和弦按实际拍 ID 解引用原生集合，提供名称、根音、29 类原生类型、音程、低音/转位、首品、弦位、横按和指法。已验证 m7 符号/和弦图、移除、横按/指法持久化及谱表隔离；显式音程输入已实现，全部自定义组合未穷举。

和弦符号独立于实际音符；歌词在指定音轨/谱表/小节/声部/拍的 0..4 行写入文本片段，空格、连字符和中文原样保存，空串清除。五行歌词、钢琴上下谱表隔离、一次撤销、导入导出、保存重开和 PDF 文本均有证据。自动指法搜索、跨拍自动分词和完整歌词排版未扩展。

### P8.4 段落和页面元数据

已实现 `gp_read_sections`、`gp_edit_section` 并扩展 `gp_presentation.page_metadata`。段落记录原生起点、名称和文本，结束由下一起点或曲尾确定；段落名称与播放跳转独立。已验证多个段落、写入/移除、结构摘要、撤销及保存重开。

页面元数据通过 `ScoreProperty` 和 `style::Stylesheet` 写入标题、作者、作曲者、版权、奇偶页眉、首页/奇偶页脚及页码文本/可见性。整组一次撤销，保存重开和两页 PDF 的文本、和弦图及页面呈现已核对；使用已校验的 `AMPainting.dll`，不修改应用全局偏好。任意刻谱样式和完整打印系统未扩展。

### P8.5 验收结果

`native/test-p8.ps1 -RenderPdf` 在最终构建通过 207 项，含开发故障注入、双客户端顺序、等待取消、提交前/后异常的观察恢复、目标隔离、GPIF、文件字节及保存重开。证据：`artifacts/native-p8-facf5a2c6a464a32ab2e726565d35b6a/verification.json`。

同一 DLL 完整原生回归通过 23 组、4375 项，包含音频 PCM、P6 和 P8，宿主退出码 0、连接描述清理完成。证据：`artifacts/regression-501eea73a4f44b96a2c8086dbf191a7e/regression.json`。核心 SHA-256 为 `52541545944091638DBFEA5BCEF7FA8C859D890DCD2976440F99681CCBA49DD4`，两份报告的源码哈希均与验收工作树一致。P8 本次为隔离开发宿主验收，既有 P7 安装包和真实安装生命周期仍保留原有发布证据，未将其当作本次新 DLL 的安装验收。具名能力及边界见 [P8 验收](COVERAGE.md#p8-验收)。

## P9：编辑面板剩余能力

P9 用于归拢当前截图及实际用户编辑流程中尚未覆盖的按钮级能力。它不把 Qt 动作枚举或控件属性修改当作曲谱编辑完成；每项能力都必须落到 Guitar Pro 原生文档模型，并有状态读回、撤销/重做和保存重开证据。P9 与 P8 的关系是：P8 负责高层编曲和语义对象的基础接口，P9 负责这些对象及其余编辑按钮在实际编辑流程中的完整可用性和边界验证；不复制第二套曲谱模型。

### P9.1 文本、和弦与记谱按钮

补齐歌词、和弦符号及和弦图、节拍/小节文本、力度标记、谱号、谱表显示和截图中其他尚无专用写入接口的记谱按钮。和弦与歌词沿用 P8 的语义对象定义，必须区分符号/文本与实际音符内容，并支持准确定位到音轨、谱表、声部、小节和节拍。

### P9.2 排版与视图编辑

补齐符杠、连音括号、分组、括号、间距、系统/页面布局和相关显示控制。修改必须证明进入宿主排版或文档模型，在保存重开及 PDF/PNG 输出中读回或呈现一致；仅改变 Qt 控件属性、临时视图状态或菜单启用状态不计为完成。

### P9.3 乐器、指法、音色与自动化

按实际需求评估任意乐器定义、打击乐演奏法、自动指法搜索、自定义音色和效果构造、效果参数名称、音量/力度/表达自动化及其他曲线编辑。已有音色或效果的选择、旁路、参数和删除不重复计算；新增能力必须说明宿主撤销栈、引擎差异和可用音源范围，并对影响声音的修改执行真实播放或 PCM 验证。

### P9.4 系统剪贴板与完整内容转移

完成系统剪贴板互通、任意音轨集合映射、完整特别粘贴过滤项、部分小节跨栏替换及键盘/弦乐/打击乐之间的可用映射。跨进程、不同调弦、变调夹和多谱表内容必须在写入前完成兼容性检查；不能用插件内部剪贴板通过代替系统剪贴板验收。

### P9.5 组合验收与支持边界

建立截图按钮到 MCP 工具、原生字段和验收证据的一对一矩阵，覆盖剩余技法组合、全部跳转组合、力度/自动化对播放的影响、多客户端和多文档隔离。每个按钮必须标记为“已实现、已验证、实验性、未实现或宿主受限”之一；不支持的宿主版本、驱动、格式或 UI 上下文必须明确拒绝。

验收标准：所有 P9 写入均在 Qt 主线程按顺序执行，核对实际模型、未保存状态、撤销/重做、保存重开和失败恢复；影响排版的操作核对 PDF/PNG，影响声音的操作核对时间线或 PCM。结果未知时保持写入阻塞，不自动重放编辑或猜测回滚成功。P9 不扩展已排除的外部常驻服务、输入模拟、前台窗口依赖或未核验宿主 ABI。

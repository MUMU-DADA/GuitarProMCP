...
插件通过 Qt 图像插件入口提交加载任务，在加载核心前校验私有接口所依赖的宿主文件。不支持的宿主或无效配置会跳过 MCP 启动并报告状态，保留 Guitar Pro 的正常使用能力。加载机制与版本有关，Guitar Pro 更新后必须重新验证。实现细节见 [自动加载方案](../native/AUTOLOAD.md)。
...
实例、安装生命周期和入口转发的最新验证结果见 [覆盖清单](COVERAGE.md)。Windows 文件关联使用 DDE 命令 `[open("%1")]`；单独再次执行 `GuitarPro.exe --open ...` 不会传递文件路径。独立 GUI 多进程列为宿主限制。

该宿主版本在启动期间快速关闭时，厂商 AMNetwork 线程可能持续等待；这是已记录的宿主限制。普通使用后的退出已单独验收，延长启动等待不代表问题已修复。

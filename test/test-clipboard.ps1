...
try {
    ...
    $expectedBars[1].voices[0].beats += [pscustomobject][ordered]@{dots=0;effects=$emptyEffects;index=2;legato=[ordered]@{destination=$false;origin=$false};native_note_value=4;notes=@();placeholder=$true;rest=$true;rhythm='Quarter;Dots=0;PriTuplet=0/0;SecTuplet=0/0';text='';tuplets=[ordered]@{primary=[ordered]@{actual=0;enabled=$false;normal=0};secondary=[ordered]@{actual=0;enabled=$false;normal=0}}}

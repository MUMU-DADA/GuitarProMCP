...
& cl /nologo /std:c++17 /EHsc /MD /O2 /utf-8 /DQT_NO_DEBUG "/I$QtDir/include" "/I$root/native" "$PSScriptRoot/p12-windows.cpp" "/Fo$output/" "/Fe$output/p12-windows.exe" /link "/LIBPATH:$QtDir/lib" Qt5Core.lib Qt5Gui.lib Qt5Widgets.lib
...
try {
    ...
    foreach ($scale in @('1','1.5')) {
        ...
        & "$output/p12-windows.exe" -platform offscreen

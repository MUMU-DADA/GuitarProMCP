...
try {
    Run-Step 'build' $shells[0] '../native/build.ps1' @()
    ...
    $sources = @(Get-ChildItem -LiteralPath @((Join-Path $root 'native'),$PSScriptRoot) -File | Where-Object Extension -In @('.h','.cpp','.ps1','.def','.json') | Get-FileHash | Select-Object Path,Hash)

...
| `guitarpro_audio_stream.h` / `audio_stream_api.h` / `wasapi_loopback_capture.h` | P14 实时流 ABI、Windows 进程级 WASAPI loopback、固定容量 ring、窗口指标、诊断和恢复 |
...
- P14 实时流契约见 [`audio_stream_api.h`](audio_stream_api.h)、[`guitarpro_audio_stream.h`](guitarpro_audio_stream.h) 和 [`wasapi_loopback_capture.h`](wasapi_loopback_capture.h)：v1 与 P13 ABI 分离，进程级 loopback 优先捕获 Guitar Pro 输出，固定容量 SPSC ring、有限值和窗口指标不持有宿主裸指针。`test/build-audio-stream-probe.ps1`、`test/test-audio-stream.ps1` 和 `test/test-audio-stream-host.ps1` 验证 FIFO、容量边界、真实 WASAPI PCM、诊断、恢复及真实 MCP 会话；`gp_audio_stream` 支持 `backend=auto|standard|asio|process_loopback|render_loopback`，并明确返回实际 capture scope。
...
# P14 实时流 ABI、WASAPI loopback、ring、指标、诊断和恢复
./test/build-audio-stream-probe.ps1
./test/test-audio-stream.ps1
./test/test-audio-stream-host.ps1 -SessionFile <session.json>

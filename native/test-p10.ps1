...
try {
    ...
    @{complete=$complete;checks=$checks;host_limited=$hostLimited;restart_persistence=$restartPersistence;new_document_defaults=$newDocumentDefaults;sources=@('guitarpro_mcp.cpp','guitarpro_audio.h','test-p10.ps1','supported-host.json') | ForEach-Object { Get-FileHash (Join-Path $PSScriptRoot $_) | Select-Object Path,Hash };plugin_sha256=(Get-FileHash "$root/.tools/native/plugins/generic/guitarpro_mcp.dll" -ErrorAction SilentlyContinue).Hash} |

...
| 保存 | 当前路径保存、`.gp` 副本、原生另存为、显式覆盖；保护其他已打开文档；锁定文件/缺失目录拒绝后保留原内容、路径和未保存状态；GPIF、撤销重做内容及保存重开 | 原生写入中途失败后的恢复及恢复失败保留备份尚未验证；其他格式和全部复杂曲谱保真度待完成；重做到保存内容后宿主仍可能标记未保存 |
...
P2 当前构建十五组回归共 2275 项通过，退出码为 0，连接描述已清理：`artifacts/regression-ad790d076ad54c85aae881bcf2b63b33/regression.json`。核心 DLL SHA-256 为 `99680BF0C99959EFE8A229346ED7EAA999AC7F2D0B6C486A3B385ECE89935D77`。本轮包含请求跟踪、关闭策略、原生取消、ZIP/GPIF 校验和保存失败后的保守未保存状态恢复；中途写入及恢复失败分支尚未注入验证。

同一构建的文档操作专项在 Windows PowerShell 5.1 下通过 50 项，宿主正常退出并清理连接描述：`artifacts/document-operations-13ae4ad7835248bf99eaa1eee823fa15/verification.json`。连接及两个客户端并发专项通过 53 项：`artifacts/instances-149583ef2ed448dcbbce0a0dad2110e4/verification.json`。二次启动文件转发、独立 GUI 多进程、真实 MCP 客户端配置重新加载仍未通过验收。

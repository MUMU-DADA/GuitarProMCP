...
| 文档状态与切换 | 独立 ID、路径、未保存状态、原生 Score 和活动文档；打开/新建/关闭请求跟踪和 64 条历史；明确保存/丢弃/取消及原生确认；关闭全部文档后重开及旧 ID 失效；损坏 ZIP/GPIF 明确拒绝；按标签顺序读取与插件重排，同名/未命名及多份未保存曲谱的身份、撤销、保存重开和关闭隔离；另存后的两种路径、标签/提示/窗口/对应菜单名称同步；新隔离宿主中的原生文档菜单目标保持 | 用户确认原生拖动不改变顺序；反复隐藏后菜单禁用上下文、完整 GPIF 语义错误、未知原生结果恢复、全部模态上下文和保存中途取消仍待验收 |
...
| 范围 | 后续需要完成的工作 |
| --- | --- |
| 文件与文档 | 导入、原生标签拖动边界、反复隐藏后的菜单上下文、完整打开失败/取消处理、未知原生结果恢复和保存中途取消；插件重排、另存后的路径/标签同步及基础保存/丢弃/取消关闭已验证 |
...
首次整套回归 `artifacts/regression-a58ec49485ff46788c1621e7b8397559/regression.json` 停于菜单名称检查：宿主保留已关闭文档的勾选动作，当前标签对应的名称实际正确。保存测试现按当前标签对应动作核验，未放宽菜单可用性专项。额外的三次恢复/隐藏观察中，第二、三次恢复后的当前文档动作仍禁用，原始状态见同一菜单宿主目录的 `menu-cycles.json`；此观察不计入通过项，菜单上下文问题仍未解决。
...
- `native/test-document-tabs.ps1`：193 项插件重排、同名及未命名文档、四份未保存曲谱、稳定身份、活动文档保持、撤销重做、可见标签坐标、模态拒绝、保存重开与关闭隔离检查。默认不验证原生菜单，另设 `-VerifyDocumentMenu` 必须实际通过的专项分支，失败不计通过。

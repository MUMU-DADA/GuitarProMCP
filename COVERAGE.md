...
| 文档状态与切换 | 独立 ID、路径、未保存状态、原生 Score 和活动文档；打开/新建/关闭请求跟踪和 64 条历史；明确保存/丢弃/取消及原生确认；关闭全部文档后重开及旧 ID 失效；损坏 ZIP/GPIF 明确拒绝；按标签顺序读取与插件重排，同名/未命名及多份未保存曲谱的身份、撤销、保存重开和关闭隔离；新隔离宿主中的原生文档菜单目标保持 | 用户确认原生拖动不改变顺序；反复隐藏后菜单禁用上下文、完整 GPIF 语义错误、未知原生结果恢复、全部模态上下文和保存中途取消仍待验收；未命名曲谱另存后的标签显示仍有缺口 |
...
| 范围 | 后续需要完成的工作 |
| --- | --- |
| 文件与文档 | 导入、原生标签拖动边界、反复隐藏后的菜单上下文、未命名曲谱另存后的标签显示、完整打开失败/取消处理、未知原生结果恢复和保存中途取消；插件重排及基础保存/丢弃/取消关闭已验证 |
...
标签早期验证发现并修正了模板名称、未保存标记前缀及另存后空提示被误当作文件路径的校验问题。原生菜单在反复隐藏/恢复的已有测试宿主中曾保持禁用，相关失败不计通过；新宿主的菜单专项不能证明所有窗口上下文已通过。用户确认原生拖动不改变顺序，插件重排不替代该项验收。未命名文档另存后标签显示可能为空、同步重排校验失败后的恢复仍待处理。

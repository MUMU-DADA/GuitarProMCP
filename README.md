...
默认端口被占用时自动分配另一个本机端口；明确设置 `GPMCP_PORT` 时冲突会报错。以生成的连接配置为准，现有客户端是否自动重读配置仍需逐个验证。`Get-McpInstances` 可发现有效实例，`New-McpSession -InstanceId ...` 明确选择目标；重连不会自动重放编辑请求。当前宿主第二次启动会退出，但转发打开文件尚未通过验证，运行中打开曲谱应使用 `gp_open`。
